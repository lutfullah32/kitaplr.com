    ...
    <div class="col-sm-12 col-xs-12">
        <div class="card">
            <div class="card-header">
                <?php
                if(isset($_GET["id"])) {
                    $kategoriadi = mysqli_query($vt, "SELECT * FROM yazarlar WHERE id=".$_GET["id"]);
                    $row=mysqli_fetch_array($kategoriadi);
                    echo '<a href="?sayfa=yazar&id='.$row["id"].'">'.$row["adsoyad"].'</a>';
                }
                ?>
            </div>
            <div class="card-body">
    ...
    <?php if(isset($_GET["id"])){ ?>
        <div class="col-sm-12 col-xs-12">
            <div class="card">
                ...
                <div class="card-body">
                    <nav>
                        <?php $query=mysqli_query($vt,"SELECT * FROM kitaplar WHERE yazar_id=".$_GET["id"]);
